...
<body>
  ...
  <div class="Descriptions">
	<p>Identifiant Alerte Livreur </p>
	<p>Message</p>
	<p>Identifiant Livreur</p>
</div>
 <?php
	session_start();
		require_once("config/connexion.php");
		connexion::connect();
		require_once("model/AlerteLivreur.php");
		...
		$tableauAlerteLivreur = AlerteLivreur::getAll();
		$i = 0;
		foreach ($tableauAlerteLivreur as $AlerteLivreur){
		$i+=1;
		echo '<div class="Commande">';
		echo	'<p>"'.$AlerteLivreur->get('ID_AlerteLivreur').'"</p>';
		echo	' <p>"'.$AlerteLivreur->get('Message').'"</p>';
		echo	'<p>"'.$AlerteLivreur->get('ID_Livreur').'"</p>';
		echo '</div>';
		}
?>
  <footer>
