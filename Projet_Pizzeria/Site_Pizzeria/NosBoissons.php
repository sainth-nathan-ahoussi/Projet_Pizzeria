		...
		<div class="catalogue">
			...
			session_start();
				...
				if (! $_SESSION['MotDePasseClient']){
					header('location: NosBoissonsGuest.php');
				}

				$id_client = $_SESSION['ID_Client'];
				$tableauBoisson = Boisson::getAll();
				...
				$i = 0;
					foreach($tableauBoisson as $Boisson){
						...
						echo '<h1>Information sur votre boisson</h1>';
						echo			'<div class="image_popup"><img src="' .$Boisson->get('imageBoisson')  . '" alt="Logo"></div>';
						echo				'<label>Description :</label>';
						echo			'<a href="#" class="modal_close">&times;</a>';
						echo		'</div>';
						echo	'</div>';
						echo	'<div class="carousel-image"><img src="' .$Boisson->get('imageBoisson')  . '"></div>';
						echo	'<div class="carousel-text">' .$Boisson->get('NomBoisson') . '</div>';
						echo	'<div class="carousel-prix">' . $Boisson->get('PrixBoisson'). '</div>';
						...
						echo		'<div class="modal_content"> ';
						echo			'<h1>Selectionnez le nombre de boisson</h1>';
						echo			'<h2>' .$Boisson->get('NomBoisson') . '</h2>';
						echo			'<div class="image_popup"><img src="' .$Boisson->get('imageBoisson')  . '" alt="Logo"></div>';
						echo			'<form method="get"> ';
						echo				'<label>Quantité :</label>';
						echo				'<input id="lfname" name="fname" type="number" min="1" max="100" />';
						echo				'<button id="#" type="submit" name="validerBoisson" value ="'.$i.'" class="">Valider</button>';
						echo			'</form>';
						echo			'<a href="#" class="modal_close">&times;</a>';
						echo		'</div>';
						echo	'</div>';
					...
					if(isset($_GET['validerBoisson'])){
						$numeroPizza =  $_GET['validerBoisson'];
						$selection = 'imageBoisson';
						$ImageProduit= Boisson::getBoissonDetails($numeroPizza, $selection);
						$Quantite =  isset($_GET['fname']) ? intval($_GET['fname']) : 0;
						$selection2 = 'PrixBoisson';
						$PrixBaseProduit = Boisson::getBoissonDetails($numeroPizza, $selection2);
